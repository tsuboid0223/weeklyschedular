...
# 依存が未インストールでもアプリが動くようにtry/exceptで読み込み
...
# カスタムCSS（見やすさ・レイアウト安定＋曜日カードのデザイン）
st.markdown("""
<style>
    ...
    /* ナビゲーションボタン行 */
    .nav-row { display:flex; gap:.5rem; justify-content:center; align-items:center; margin: .4rem 0 .9rem 0; }
...
# D&Dボード（streamlit-sortables、バージョン差に対応）
def render_dnd_board(week_dates):
    ...
    # バージョン差に対応した可変引数（keyは渡さない：一部版でUIに表示される問題回避）
    kwargs = {"multi_containers": True, "direction": "horizontal"}
    try:
        params = inspect.signature(sort_items).parameters
        if "styles" in params:
            kwargs["styles"] = {
                "container": {"minHeight": "220px", "backgroundColor":"#f8fafc", "border":"2px dashed #e2e8f0",
...
def main():
    ...
    # 週移動ボタン
    with st.container():
        colp, colc, coln = st.columns([1, 1, 1])
        with colp:
            if st.button("⬅ 前の週"):
                goto_prev_week()
        with colc:
    ...
    for i, (date, col, weekday) in enumerate(zip(week_dates, cols, weekdays)):
        with col:
            ...
            st.markdown(
                ...
                f'  <div class="dc-head"><div class="dc-name">{weekday}</div><div class="dc-date">{format_date_jp(date)}</div></div>'
                f'  <div class="dc-body">', unsafe_allow_html=True
            )
